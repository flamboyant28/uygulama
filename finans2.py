...
def read_csv(path, cols):
    ...
    try:
        # Hem virgüllü hem noktalı virgüllü CSV'leri otomatik algıla.
        df = pd.read_csv(path, encoding='utf-8-sig', sep=None, engine='python')
    except UnicodeDecodeError:
        df = pd.read_csv(path, encoding='cp1254', sep=None, engine='python')
    for c in cols:
...
with st.sidebar:
    ...
    if st.button('📥 Piyasa verisini ekle',use_container_width=True):
        ...
        try:
            try:
                raw=pd.read_csv(up,encoding='utf-8-sig',sep=None,engine='python')
            except UnicodeDecodeError:
                raw=pd.read_csv(up,encoding='cp1254',sep=None,engine='python')
    ...
    st.divider()
    st.subheader('🧹 Piyasa Verisini Temizle')
    st.caption('Bu işlem sadece hisseler.csv içindeki piyasa geçmişini siler. islemler.csv ve AL/SAT işlemlerin silinmez.')
    clear_market_confirm = st.checkbox('Piyasa verilerinin tamamını silmek istiyorum', key='confirm_clear_market')
    if st.button('🗑️ Piyasa Verilerini Temizle', use_container_width=True, disabled=not clear_market_confirm):
        pd.DataFrame(columns=MARKET_COLS).to_csv(HISSELER,index=False,encoding='utf-8-sig')
        st.success('Piyasa verileri temizlendi. İşlem geçmişin korunuyor.')
        st.rerun()
...
with t2:
    ...
    symbol=c1.selectbox('Hisse',opts,index=None,placeholder='Hisse ara...',key='trade_symbol') if opts else c1.text_input('Hisse',key='trade_symbol_text').upper()
...
with t4:
    ...
    if latest.empty: st.info('Veri yok.')
        ...
        st.subheader('📈 Fiyat Geçmişi'); opts=sorted(market.Kod.dropna().unique()); s=st.selectbox('Hisse',opts,index=None,placeholder='Hisse ara...',key='price_history_symbol')
        if s:
