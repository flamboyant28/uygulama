...
def sehir_ara(q):
    """Open-Meteo Geocoding API — rate limit yok, hızlı, güvenilir"""
    try:
        r = requests.get(
            "https://geocoding-api.open-meteo.com/v1/search",
            params={"name": q, "count": 5, "language": "tr", "format": "json"},
            timeout=15
        )
        d = r.json()
        sonuclar = d.get("results", [])
        # Nominatim formatına dönüştür (lat/lon/display_name)
        return [
            {
                "lat":          s["latitude"],
                "lon":          s["longitude"],
                "display_name": f"{s['name']}, {s.get('admin1','')} {s.get('country','')}".strip(", "),
            }
            for s in sonuclar
        ]
    except Exception as e:
        st.error(f"Şehir arama hatası: {e}")
        return []
...
with st.sidebar:
    ...
    for s in ["Üsküdar","Çankaya","Seyhan","Görele",
              "Basra","Sukkur","He'an","Tokyo","Helsinki","Yakutsk","Reykjavik","Mykines"]:
        ...
        def ters_cevir(lat, lon):
            try:
                r = requests.get("https://nominatim.openstreetmap.org/reverse",
                    ...
                    headers={
                        "User-Agent": "HavaDurumu-Dashboard/4.0 (streamlit-app)",
                ...
                return f"{ad}, {ulke}"
            except: return f"{lat:.4f}°N, {lon:.4f}°E"
