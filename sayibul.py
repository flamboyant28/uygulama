...
# ─── Çekiliş Ekleme Arayüzü ──────────────────────────────────────────────────
GUNCELLE_CFG = {
    "🎯 Sayısal Loto": {
        "dosya": "sayisal_loto_arsiv.json",
        "ana": 6, "havuz": 90, "renk": "#1a4fa0",
        "etiket": ["T1","T2","T3","T4","T5","T6"],
        "joker": True, "joker_hav": 90,
        "super": True, "super_hav": 90,
        "bonus": False,
    },
    "⭐ Süper Loto": {
        "dosya": "super_loto_arsiv.json",
        "ana": 6, "havuz": 60, "renk": "#7d3c98",
        "etiket": ["T1","T2","T3","T4","T5","T6"],
        "joker": False, "super": False, "bonus": False,
    },
    "🔵 Şans Topu": {
        "dosya": "sans_topu_arsiv.json",
        "ana": 5, "havuz": 34, "renk": "#16a085",
        "etiket": ["T1","T2","T3","T4","T5"],
        "joker": False, "super": False,
        "bonus": True, "bonus_hav": 14,
    },
    "🔴 On Numara": {
        "dosya": "on_numara_arsiv.json",
        "ana": 22, "havuz": 80, "renk": "#c0392b",
        "etiket": [str(i) for i in range(1, 23)],
        "joker": False, "super": False, "bonus": False,
    },
}

def _top_mini(n, renk):
    return (f'<span style="display:inline-flex;width:28px;height:28px;border-radius:50%;'
            f'background:{renk};color:#fff;align-items:center;justify-content:center;'
            f'font-size:11px;font-weight:700;margin:1px">{n}</span>')

def _satir(row, cfg):
    html = f'<span style="color:#888;font-size:11px;margin-right:6px">{row["tarih"]}</span>'
    for n in row["sayilar"]:
        html += _top_mini(n, cfg["renk"])
    if cfg.get("joker") and row.get("joker"):
        html += '<span style="color:#aaa;margin:0 3px;font-size:11px">J</span>' + _top_mini(row["joker"], "#e67e22")
    if cfg.get("super") and row.get("super"):
        html += '<span style="color:#aaa;margin:0 3px;font-size:11px">S</span>' + _top_mini(row["super"], "#b7950b")
    if cfg.get("bonus") and row.get("bonus"):
        html += '<span style="color:#aaa;margin:0 3px;font-size:11px">+</span>' + _top_mini(row["bonus"], "#e6a817")
    return html

def cekilis_ekle_sayfasi():
    import json as _j
    from datetime import date as _date

    st.caption("Yeni çekiliş ekle → JSON güncelle → İndir → GitHub'a push et.")
    alt_tabs = st.tabs(list(GUNCELLE_CFG.keys()))

    for alt_tab, (oyun_adi, cfg) in zip(alt_tabs, GUNCELLE_CFG.items()):
        with alt_tab:
            dosya = cfg["dosya"]
            rows = []
            if _os.path.exists(dosya):
                with open(dosya, encoding="utf-8") as _f:
                    rows = _j.load(_f)
            rows_s = sorted(rows, key=lambda r: _dt.strptime(r["tarih"], "%d/%m/%Y"))

            m1, m2, m3 = st.columns(3)
            m1.metric("Toplam", len(rows_s))
            if rows_s:
                m2.metric("İlk", rows_s[0]["tarih"])
                m3.metric("Son", rows_s[-1]["tarih"])

            st.divider()

            with st.form(key=f"ekle_{oyun_adi}"):
                ct, _ = st.columns([2, 4])
                with ct:
                    tarih_val = st.date_input("Çekiliş tarihi", value=_date.today(),
                        format="DD/MM/YYYY", key=f"g_tarih_{oyun_adi}")

                st.caption(f"Ana sayılar (1–{cfg['havuz']})")
                sayilar_input = []
                grup = 6
                for si in range(0, cfg["ana"], grup):
                    cols_g = st.columns(min(grup, cfg["ana"] - si))
                    for ci, col in enumerate(cols_g):
                        gi = si + ci
                        with col:
                            n = st.number_input(cfg["etiket"][gi], min_value=1,
                                max_value=cfg["havuz"], value=None, step=1,
                                key=f"g_s{gi}_{oyun_adi}", placeholder="—")
                            sayilar_input.append(n)

                joker_val = super_val = bonus_val = None
                if cfg.get("joker"):
                    st.caption(f"Joker (1–{cfg['joker_hav']})")
                    cj, _ = st.columns([1, 5])
                    with cj:
                        joker_val = st.number_input("Joker", 1, cfg["joker_hav"],
                            value=None, step=1, key=f"g_joker_{oyun_adi}", placeholder="—")
                if cfg.get("super"):
                    st.caption(f"SüperStar (1–{cfg['super_hav']})")
                    cs, _ = st.columns([1, 5])
                    with cs:
                        super_val = st.number_input("SüperStar", 1, cfg["super_hav"],
                            value=None, step=1, key=f"g_super_{oyun_adi}", placeholder="—")
                if cfg.get("bonus"):
                    st.caption(f"Şans Topu (1–{cfg['bonus_hav']})")
                    cb, _ = st.columns([1, 5])
                    with cb:
                        bonus_val = st.number_input("Şans Topu", 1, cfg["bonus_hav"],
                            value=None, step=1, key=f"g_bonus_{oyun_adi}", placeholder="—")

                ekle_btn = st.form_submit_button("✅ Ekle", type="primary", use_container_width=True)

            if ekle_btn:
                tarih_str = tarih_val.strftime("%d/%m/%Y")
                sayilar_temiz = sorted([int(n) for n in sayilar_input if n is not None])
                hatalar = []
                if len(sayilar_temiz) < cfg["ana"]:
                    hatalar.append(f"Tüm {cfg['ana']} sayıyı girin.")
                if len(set(sayilar_temiz)) != len(sayilar_temiz):
                    hatalar.append("Sayılar birbirinden farklı olmalı.")
                if cfg.get("joker") and joker_val and int(joker_val) in sayilar_temiz:
                    hatalar.append("Joker, ana sayılardan farklı olmalı.")
                if tarih_str in [r["tarih"] for r in rows]:
                    hatalar.append(f"{tarih_str} zaten mevcut!")
                if hatalar:
                    for h in hatalar: st.error(h)
                else:
                    yeni = {"tarih": tarih_str, "sayilar": sayilar_temiz,
                            "sistem": "yeni" if tarih_val >= _date(2020, 8, 3) else "eski", "sira": 0}
                    if cfg.get("joker") and joker_val: yeni["joker"] = int(joker_val)
                    if cfg.get("super") and super_val: yeni["super"] = int(super_val)
                    if cfg.get("bonus") and bonus_val: yeni["bonus"] = int(bonus_val)
                    rows.append(yeni)
                    rows.sort(key=lambda r: _dt.strptime(r["tarih"], "%d/%m/%Y"))
                    for i, r in enumerate(rows): r["sira"] = i + 1
                    with open(dosya, "w", encoding="utf-8") as _f:
                        _j.dump(rows, _f, ensure_ascii=False, separators=(",", ":"))
                    st.success(f"✅ {tarih_str} eklendi! Toplam: {len(rows)} çekiliş")
                    st.markdown(_satir(yeni, cfg), unsafe_allow_html=True)

            st.divider()
            n_gos = st.slider("Son kaç çekiliş?", 5, 30, 10, key=f"g_n_{oyun_adi}")
            for r in sorted(rows, key=lambda r: _dt.strptime(r["tarih"], "%d/%m/%Y"), reverse=True)[:n_gos]:
                st.markdown(_satir(r, cfg), unsafe_allow_html=True)

            st.divider()
            st.download_button(
                label=f"⬇️ {dosya} indir ({len(rows)} çekiliş)",
                data=_j.dumps(rows, ensure_ascii=False, indent=2).encode("utf-8"),
                file_name=dosya, mime="application/json",
                use_container_width=True, key=f"g_indir_{oyun_adi}"
            )
            with st.expander("🗑️ Kayıt Sil"):
                tarih_l = [r["tarih"] for r in sorted(rows, key=lambda r: _dt.strptime(r["tarih"], "%d/%m/%Y"), reverse=True)]
                if tarih_l:
                    sil_t = st.selectbox("Tarih:", tarih_l, key=f"g_sil_{oyun_adi}")
                    if st.button("🗑️ Sil", key=f"g_sil_btn_{oyun_adi}", type="secondary"):
                        rows = [r for r in rows if r["tarih"] != sil_t]
                        for i, r in enumerate(rows): r["sira"] = i + 1
                        with open(dosya, "w", encoding="utf-8") as _f:
                            _j.dump(rows, _f, ensure_ascii=False, separators=(",", ":"))
                        st.success(f"✅ {sil_t} silindi.")
                        st.rerun()
...
tab1,tab2,tab3,tab4,tab5,tab6=st.tabs(list(OYUNLAR.keys()) + ["🔍 Kolon Analizi", "📝 Çekiliş Ekle"])
