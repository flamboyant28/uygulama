...
# ── Session state: [(il, ilçe), ...] + segment km override'ları ──────────────
if "duraks" not in st.session_state:
    st.session_state.duraks = [("İstanbul", "Kadıköy"), ("Ankara", "Çankaya")]
if "km_ov" not in st.session_state:
    st.session_state.km_ov = [0]          # len = len(duraks)-1

def _sync_km_ov():
    """Durak sayısıyla km_ov listesini senkronize tutar."""
    n = len(st.session_state.duraks) - 1
    while len(st.session_state.km_ov) < n:
        st.session_state.km_ov.append(0)
    while len(st.session_state.km_ov) > n:
        st.session_state.km_ov.pop()
...
with st.sidebar:
    ...
    to_remove = None
    n_duraks = len(st.session_state.duraks)

    for i in range(n_duraks):
        il, ilce = st.session_state.duraks[i]
        if i == 0:              lbl = "🚩 Kalkış"
        elif i == n_duraks - 1: lbl = "🏁 Varış"
        else:                   lbl = f"📍 {i}. Durak"
        ...
        # Segmentler arası km override (son duraktan sonra gösterme)
        if i < n_duraks - 1:
            ...
            ov = st.number_input(
                f"↕ Segment km (0 = tahmini {auto_km} km)",
                value=int(st.session_state.km_ov[i]),
                min_value=0, step=1,
                key=f"km_ov_{i}",
                help="Google Maps'ten aldığın gerçek km'yi gir. 0 bırakırsan koordinattan hesaplanır."
            )
    ...
    # Segmentleri hesapla (override varsa onu, yoksa haversine kullan)
    segments = []
    for i in range(len(st.session_state.duraks) - 1):
        ...
        kaynak = "📍" if ov > 0 else "📐"
        segments.append((f"{a_il}/{a_ilce}", f"{b_il}/{b_ilce}", km, kaynak))
    ...
    if segments:
        ...
        st.caption("📍 Manuel km  ·  📐 Koordinat tahmini")
    ...
    fiyat    = st.number_input("Benzin Fiyatı (TL/L)", value=64.88, min_value=0.01, step=0.5, format="%.2f")
...
if len(segments) > 1:
    ...
    def seg_sure_h(km):
        """Bir segmentin toplam süresi (saat cinsinden, molalar dahil)."""
        h = km / hiz
        return h + int(h / mola_araligi) * mola_suresi / 60

    # Kümülatif saat hesabı: her segmentin çıkış/varış saatini bul
    current_dt = datetime.combine(datetime.today(), cikis)
    rows = []
    for a, b, km, src in segments:
        dep_str  = current_dt.strftime("%H:%M")
        sure_h   = seg_sure_h(km)
        arr_dt   = current_dt + timedelta(hours=sure_h)
        ...
        rows.append({
            ...
            "Çıkış":          dep_str,
            "Varış":          arr_str,
            "Sürüş Süresi":   fmt_sure(km / hiz),
            "Mola":           f"{mol_dk}dk" if mol_dk else "—",
            "Toplam Süre":    fmt_sure(sure_h),
            "Yakıt Maliyeti": f"{km * km_maliyet:,.0f} ₺",
        })
        current_dt = arr_dt   # bir sonraki segmentin çıkışı bu segmentin varışı
