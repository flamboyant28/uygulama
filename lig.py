...
with tab4:
    ...
    with col_dl:
        if os.path.exists(DATA_FILE):
            with open(DATA_FILE, "r", encoding="utf-8") as f:
                json_str = f.read()
            st.download_button(
                label="⬇️  JSON'u İndir",
                data=json_str,
                file_name="lig_data.json",
    ...
    with col_up:
        ...
        if uploaded:
            try:
                ...
                # Temel doğrulama
                if "teams" in loaded and "fixtures" in loaded and "n" in loaded:
    ...
    if st.button("💾  Takım İsimlerini Kaydet", type="primary", use_container_width=True):
        ...
        for idx, new_name in new_names:
            if not new_name:
                st.warning(f"Takım {idx+1} ismi boş bırakılamaz, atlandı.")
                continue
            old_name = data["teams"][idx]
            if old_name != new_name:
                # Fikstürdeki maçlarda da güncelle
                for m in data["fixtures"]:
                    if m["home"] == old_name:
                        m["home"] = new_name
                    if m["away"] == old_name:
    ...
    with col_r2:
        if st.button("💣 Ligi Tamamen Sıfırla", use_container_width=True, type="secondary"):
            if st.session_state.get("confirm_reset"):
                new_data = default_data(N)
                st.session_state.data = new_data
                st.session_state.confirm_reset = False
                save_data(new_data)
                st.success("Lig sıfırlandı.")
                st.rerun()
            else:
                st.session_state.confirm_reset = True
                st.warning("⚠️ Emin misin? Tüm veriler silinecek. Onaylamak için tekrar bas.")
