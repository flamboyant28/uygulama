...
def _font_yukle():
    ...
    # .py ile aynı dizin + sistem yolları
    _here = os.path.dirname(os.path.abspath(__file__))

    adaylar = [
        # Yerel dizin — arial.ttf / arialbd.ttf (py ile aynı klasör)
        ("PDF_Arial", "PDF_Arial_Bold",
         os.path.join(_here, "arial.ttf"),
         os.path.join(_here, "arialbd.ttf")),
        # Windows sistem — Arial
        ("PDF_Arial", "PDF_Arial_Bold",
         r"C:\Windows\Fonts\arial.ttf",
         r"C:\Windows\Fonts\arialbd.ttf"),
        ("PDF_Arial", "PDF_Arial_Bold",
         os.path.expandvars(r"%SystemRoot%\Fonts\arial.ttf"),
         os.path.expandvars(r"%SystemRoot%\Fonts\arialbd.ttf")),
        # Yerel dizin — calibri
        ("PDF_Calibri", "PDF_Calibri_Bold",
         os.path.join(_here, "calibri.ttf"),
         os.path.join(_here, "calibrib.ttf")),
        # Windows sistem — Calibri
        ("PDF_Calibri", "PDF_Calibri_Bold",
         r"C:\Windows\Fonts\calibri.ttf",
         r"C:\Windows\Fonts\calibrib.ttf"),
        # Linux / Mac — DejaVu
        ("PDF_DejaVu", "PDF_DejaVu_Bold",
         "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf",
         "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"),
    ]

    for f_n, f_b, path_n, path_b in adaylar:
        if os.path.exists(path_n) and os.path.exists(path_b):
            try:
                ...
                # Font family kaydı — bold/italic eşlemesi için zorunlu
                registerFontFamily(f_n, normal=f_n, bold=f_b)
                return f_n, f_b
            except Exception:
                continue
    raise RuntimeError("Hiçbir uygun font bulunamadı. arial.ttf ve arialbd.ttf "
                       "dosyalarını uygulama klasörüne kopyalayın.")
