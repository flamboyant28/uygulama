...
# Hıza göre yakıt tüketim düzeltme — fizik tabanlı formül
# Kaynak: ICCT gerçek dünya verileri kalibrasyonu (130 km/h ≈ +25% referans)
# Formül: C(v) = C_ref × [α + (1-α) × (v/v_ref)²]
# α = 0.77: yuvarlanma direnci payı — aerodinamik olmayan sabit kayıpları temsil eder
# v_ref = 90 km/h: üretici test hızı (girilecek tüketim değerinin referansı)
# Doğrulama: 200 km/h için 8 L/100km → 15.3 L (gerçekçi, "100 L" olmaz)
_HIZ_ALFA = 0.77   # Kalibre edilmiş sabit direnç payı
_HIZ_REF  = 90.0   # Referans hız (km/h)

def hiz_katsayi_hesapla(hiz_kmh: float) -> float:
    """ICCT verilerine göre kalibre edilmiş hibrit formül.
    90 km/h = 1.00 referans. Girilen tüketim değerinin baz hızıdır.
    > 90 km/h: aerodinamik direnç fizik formülü (v²)
    < 90 km/h: şehir/düşük hız cezası — dur-kalk, düşük vites verimsizliği
    Kaynak: ICCT Real-World Fuel Consumption (2023), EEA NEDC/WLTP analizi"""
    if hiz_kmh >= _HIZ_REF:
        # Yüksek hız: aereodinamik direnç egemen (v² formülü)
        return _HIZ_ALFA + (1 - _HIZ_ALFA) * (hiz_kmh / _HIZ_REF) ** 2
    else:
        # Düşük hız: şehir etkisi — 50 km/h ≈ +30%, 70 km/h ≈ +8%
        # Lineer interpolasyon: 50→1.30, 70→1.08, 90→1.00
        if hiz_kmh <= 50:  return 1.30
        elif hiz_kmh <= 70:
            return 1.30 + (1.08 - 1.30) * (hiz_kmh - 50) / (70 - 50)
        else:
            return 1.08 + (1.00 - 1.08) * (hiz_kmh - 70) / (90 - 70)
...
with st.sidebar:
    ...
    hiz_kat  = hiz_katsayi_hesapla(hiz)
    hiz_pct  = (hiz_kat - 1.0) * 100
    hiz_acik = f"{hiz_pct:+.0f}% ({hiz} km/h)" if hiz != 90 else "Referans hız (0%)"

    # Toplam düzeltme: önce hız etkisi, sonra hava, sonra yük
    tuketim_d    = tuketim * hiz_kat * hava_kat + yuk_ek
    duzeltme_pct = (tuketim_d / tuketim - 1) * 100

    st.markdown(f"""<div class="etki-box">
    🚗 Hız ({hiz} km/h): {hiz_acik}<br>
    🌡 Hava: {hava_acik}<br>
...
with tab_ozet:
    ...
    st.markdown('<div class="section-header">📐 Hıza Göre Menzil & Süre Analizi</div>', unsafe_allow_html=True)
    st.caption("Girilen tüketim değeri 90 km/h referans alınır. Hava & yük etkisi dahildir.")

    _hiz_listesi = [50, 70, 80, 90, 100, 110, 120, 130, 150]
    _hiz_rows = []
    for _v in _hiz_listesi:
        _hk      = hiz_katsayi_hesapla(_v)
        _tuk_v   = tuketim * _hk * hava_kat + yuk_ek   # bu hızdaki tüketim
        ...
        _row = {
            "Hız (km/h)":       f"{'▶ ' if _v == hiz else ''}{_v}",
            "Tüketim (L/100km)":f"{_tuk_v:.1f}",
            "Hız Etkisi":       f"{(_hk-1)*100:+.0f}%",
            "Depo Menzili (km)":f"{_menzil:.0f}",
            "Sürüş Süresi":     fmt_sure(_sure_h),
            "Mola":             f"{_mol_s}×{mola_suresi}dk" if _mol_s > 0 else "—",
            "Toplam Süre":      fmt_sure(_toplam),
            "Varış Saati":      _varis_v,
        }
        _hiz_rows.append(_row)

    _hiz_df = pd.DataFrame(_hiz_rows)
    st.dataframe(_hiz_df, hide_index=True, use_container_width=True)
