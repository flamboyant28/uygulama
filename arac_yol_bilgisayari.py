...
if "fiyatlar" not in st.session_state:
    st.session_state.fiyatlar = {"benzin":64.88,"dizel":55.0,"lpg":25.0,"ts":None}
...
# ── Güncel yakıt fiyatı çekme ─────────────────────────────────────────────────
def fiyat_cek():
    """Anthropic API + web_search ile güncel fiyat çeker."""
    try:
        ...
        resp = client.messages.create(
            ...
            messages=[{"role":"user","content":
                "Türkiye bugünkü akaryakıt fiyatları kaç TL/litre? "
                "Sadece şu formatta ver, başka hiçbir şey yazma: "
                "benzin:XX.XX dizel:XX.XX lpg:XX.XX"}]
        )
        text = " ".join(b.text for b in resp.content if hasattr(b,"text"))
        benzin = re.search(r'benzin[:\s]+([\d,.]+)', text, re.I)
        dizel  = re.search(r'dizel[:\s]+([\d,.]+)',  text, re.I)
        lpg    = re.search(r'lpg[:\s]+([\d,.]+)',    text, re.I)
        if benzin: st.session_state.fiyatlar["benzin"] = float(benzin.group(1).replace(",","."))
        if dizel:  st.session_state.fiyatlar["dizel"]  = float(dizel.group(1).replace(",","."))
        if lpg:    st.session_state.fiyatlar["lpg"]    = float(lpg.group(1).replace(",","."))
        st.session_state.fiyatlar["ts"] = datetime.now().strftime("%d.%m.%Y %H:%M")
        return True
    except Exception as e:
        st.sidebar.error(f"Fiyat çekilemedi: {e}")
        return False
...
with st.sidebar:
    ...
    if st.button("🔄 Güncel Fiyat Çek", use_container_width=True,
                 help="ANTHROPIC_API_KEY ortam değişkeni gereklidir"):
        with st.spinner("Fiyatlar aranıyor..."):
            if fiyat_cek():
                st.success(f"Güncellendi: {st.session_state.fiyatlar['ts']}")

    if st.session_state.fiyatlar["ts"]:
        st.caption(f"Son güncelleme: {st.session_state.fiyatlar['ts']}")
...
with tab_rapor:
    ...
    st.divider()
    st.caption("⛽ Güncel yakıt fiyatı için ANTHROPIC_API_KEY ortam değişkenini ayarlayın.")
