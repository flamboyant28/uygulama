...
# Font kayıt: Calibri (Windows) → DejaVu (Linux/Mac) → Helvetica (fallback)
def _font_yukle():
    import os, platform
    adaylar = [
        # Windows — Arial (öncelikli)
        ("Arial",    "Arial-Bold",
        ...
        ("Calibri",  "Calibri-Bold",
         os.path.expandvars(r"%SystemRoot%\Fonts\calibri.ttf"),
         os.path.expandvars(r"%SystemRoot%\Fonts\calibrib.ttf")),
        # Linux / Mac — DejaVu
        ("DejaVu",   "DejaVu-Bold",
    ...
    for f_normal, f_bold, path_n, path_b in adaylar:
        if os.path.exists(path_n) and os.path.exists(path_b):
            try:
                pdfmetrics.registerFont(TTFont(f_normal, path_n))
                pdfmetrics.registerFont(TTFont(f_bold,   path_b))
