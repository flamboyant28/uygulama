...
t2.metric("Mola Süresi",   mola_str,   "Her 2 saatte 1 × 15dk",              delta_color="off")
...
if len(segments) > 1:
    ...
    def seg_sure(km):
        h = km / hiz
        mol = int(h / mola_araligi) * mola_suresi
        return fmt_sure(h + mol / 60)

    seg_df = pd.DataFrame(
        [(f"{a} → {b}", f"{km} km ({src})", f"{km * km_maliyet:,.0f} ₺",
          fmt_sure(km / hiz), f"{int(km/hiz/mola_araligi)*mola_suresi}dk", seg_sure(km))
         for a, b, km, src in segments],
